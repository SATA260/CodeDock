...
type (
	...
	Result = tool.Result
	// AgentMode 是本轮运行模式：ask_for_approval / auto_approve / yolo。
	AgentMode = pkgagent.AgentMode
)
...
type Method struct {
	...
	Capabilities     []string        // read / write / memory；空表示不额外要能力
	RequiresApproval bool            // true 时 ask_for_approval 模式会先停下来等人批
}
